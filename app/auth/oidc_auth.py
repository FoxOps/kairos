...
class OIDCAuthLib:
    ...
    def _configure_oauth(self):
        ...
        try:
            ...
            issuer_url = str(OIDCConfig.ISSUER).rstrip('/')
            ...
            self.oidc_client = self.oauth.register(
                ...
                client_kwargs={
                    'scope': str(OIDCConfig.SCOPE),
                },
            )
            
            # Mettre à jour les informations du client avec les credentials
            self.oidc_client.client_id = str(OIDCConfig.CLIENT_ID)
            self.oidc_client.client_secret = str(OIDCConfig.CLIENT_SECRET)
            
            logger.info("Client OIDC Authlib configuré avec succès")
            logger.debug(f"Client ID: {OIDCConfig.CLIENT_ID}")
            logger.debug(f"Issuer: {str(OIDCConfig.ISSUER)}")  # ✅ Corrigé : conversion en str
    ...
    def get_authorization_url(self, state=None, nonce=None):
        ...
        try:
            ...
            params = {
                'response_type': 'code',
                'client_id': str(OIDCConfig.CLIENT_ID),
                'redirect_uri': str(OIDCConfig.REDIRECT_URI),
                'scope': str(OIDCConfig.SCOPE),
                'state': state,
                'nonce': nonce,
            }
    ...
    def exchange_code_for_token(self, code):
        ...
        try:
            ...
            data = {
                ...
                'redirect_uri': str(OIDCConfig.REDIRECT_URI),
                'client_id': str(OIDCConfig.CLIENT_ID),
                'client_secret': str(OIDCConfig.CLIENT_SECRET),
            }
