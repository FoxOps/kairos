...
def get_json_from_env(env_var: str, default: Optional[Any] = None) -> Any:
    """
    ...
    try:
        ...
        logging.warning(
            f"Invalid JSON value for {env_var}: '{value}'. Using default"
        )
        return default


def get_database_type(database_uri: Optional[str] = None) -> str:
    """
...
class Config:
    ...
    RATE_LIMIT_DEFAULT: str = os.environ.get("RATE_LIMIT_DEFAULT") or "200 per day, 50 per hour"
    ...
    LOG_FORMAT: str = os.environ.get("LOG_FORMAT") or "%(asctime)s - %(name)s - %(levelname)s - %(message)s"
    
    # Custom SQLAlchemy Engine Options
    custom_engine_options: Dict[str, Any] = get_json_from_env("SQLALCHEMY_ENGINE_OPTIONS") or {}
    ...
    def get_database_type(database_uri: Optional[str] = None) -> str:
        ...
        return get_database_type(
            database_uri or Config.SQLALCHEMY_DATABASE_URI
        )
