...
class LeaveService:
    ...
    def delete_filtered(
        ...
        "delete filtered result" action. `ids`: backs the checkbox
        row-selection "delete selection" action - same entrypoint, just
        another filter dimension. Unlike Shift/OnCall's delete_filtered()
        (a single bulk SQL DELETE), this loops delete_leave() per
        matching leave: each deletion triggers a shift rebalance
        (_rebalance_after_leave) that a raw bulk DELETE would skip,
        leaving stale shift assignments for the affected users/dates.
        Slower for a large filtered set, but correctness over speed
        here - the confirm dialog shows the count before an admin
        commits to it."""
