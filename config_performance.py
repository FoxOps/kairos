...
class CacheSettings:
    ...
    route_cache_timeout: dict[str, int] = field(
        default_factory=lambda: {
            "index": 60,
            "schedule": 120,
            "oncall": 120,
...
class QueryOptimizationSettings:
    ...
    default_eager_loads: dict[str, list[str]] = field(
        default_factory=lambda: {
            "User": ["group"],
    ...
    auto_indexes: list[dict[str, Any]] = field(
        default_factory=lambda: [
...
class PerformanceConfig:
    ...
    query_optimization: QueryOptimizationSettings = field(
        default_factory=QueryOptimizationSettings
    )
    ...
    def from_env(cls) -> "PerformanceConfig":
        ...
        def get_list(env_var: str, default: list | None = None) -> list:
            value = os.environ.get(env_var, "")
            if not value:
                return default or []
            try:
    ...
    def from_dict(cls, data: dict[str, Any]) -> "PerformanceConfig":
        ...
        pagination_settings = PaginationSettings(
            ...
            per_page_options=pagination_data.get(
    ...
    def configure(self, app) -> None:
        ...
        app.config["PAGINATION_PER_PAGE_OPTIONS"] = json.dumps(
            self.pagination.per_page_options
        )
        ...
        app.config["LAZY_LOAD_DEFAULT_BATCH_SIZE"] = (
            self.lazy_loading.default_batch_size
        )
...
def apply_performance_config(
    app, config: PerformanceConfig | None = None
) -> PerformanceConfig:
    """
