...
def setup_database():
    ...
    No deployment predates Alembic's adoption anymore, so this is just
    `flask_migrate.upgrade()` - every migration applies in order on a
    brand-new database, and only what's missing on an existing one.
    """
