...
def migrate_database():
    ...
    if inspector.has_table('shift') and 'shift_type' in [col['name'] for col in inspector.get_columns('shift')]:
        print("💾 Migration des shifts existants...")
        
        # Sauvegarder les données existantes
        try:
            existing_shifts = db.session.execute(
                db.text("SELECT id, user_id, shift_type, start_time, end_time, date FROM shift")
            ).fetchall()
            ...
            db.session.commit()
            print(f"✅ {len(existing_shifts)} shifts migrés avec succès vers la nouvelle structure.")
