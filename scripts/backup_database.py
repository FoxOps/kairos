...
def detect_db_path(config: BackupConfig) -> Optional[str]:
...
def create_local_backup(db_path: str, backup_path: str, compress: bool = True) -> Tuple[bool, str]:
    """
...
def verify_backup(db_path: str, backup_path: str, compress: bool = False) -> Tuple[bool, str]:
    """
...
def upload_to_s3(
    ...
    """
    if not S3_AVAILABLE:
        return False, "La bibliothèque boto3 n'est pas installée. Installez-la avec: pip install boto3"
    ...
    try:
        ...
        try:
            ...
            if e.response['Error']['Code'] == '404':
                ...
                return False, f"Accès refusé au bucket: {bucket}. Vérifiez vos permissions."
            else:
        ...
        return False, "Identifiants S3 manquants. Configurez BACKUP_S3_ACCESS_KEY et BACKUP_S3_SECRET_KEY"
    except ClientError as e:
        return False, f"Erreur S3: {e.response['Error']['Code']} - {e.response['Error']['Message']}"
    except Exception as e:
...
def cleanup_local_backups(config: BackupConfig, logger: logging.Logger) -> Tuple[int, str]:
    """
        ...
        for backup in backup_files:
            if backup['mtime'] < cutoff_timestamp:
                try:
                    ...
                    logger.info(f"Suppression: {backup['filename']} ({datetime.fromtimestamp(backup['mtime']).strftime('%Y-%m-%d')})")
                    deleted_count += 1
                except Exception as e:
                    logger.error(f"Erreur lors de la suppression de {backup['filename']}: {str(e)}")
            ...
            for obj in page['Contents']:
                ...
                if age > config.retention_days:
                    try:
                        ...
                        logger.error(f"Erreur lors de la suppression S3 de {key}: {str(e)}")
...
def create_backup(config: BackupConfig, logger: logging.Logger) -> dict:
    ...
    if config.local_enabled:
        ...
        if success:
            ...
            if config.verify_backup:
                verify_success, verify_message = verify_backup(db_path, backup_path, config.compress)
    ...
    if config.s3_enabled and config.s3_bucket:
        ...
        if results['local'] and results['local']['success']:
            ...
            success, message = upload_to_s3(backup_path, config.s3_bucket, s3_key, config, logger)
            results['s3'] = {'success': success, 'bucket': config.s3_bucket, 'key': s3_key, 'message': message}
            ...
            try:
                success, message = create_local_backup(db_path, tmp_backup_path, config.compress)
                if success:
    ...
    results['success'] = (
        (not config.local_enabled or (results['local'] and results['local']['success'])) and
        (not config.s3_enabled or (results['s3'] and results['s3']['success']))
    )
...
Variables d'environnement:
  ...
  Voir scripts/backup_config.py pour la liste complète.
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Effectuer une sauvegarde S3'
    )
    parser.add_argument(
        '--verify',
        action='store_true',
    ...
    if args.config:
        ...
        try:
            ...
            print(f"Erreur de chargement de la configuration: {str(e)}", file=sys.stderr)
            sys.exit(1)
    else:
    ...
    if args.list:
        ...
        for backup in backups['local']:
            print(f"  {backup['filename']} ({backup['size']} octets, {backup['modified']})")
