...
def find_duplicate_functions(directory, min_lines=5):
    ...
    for root, dirs, filenames in os.walk(directory):
        ...
        dirs[:] = [d for d in dirs if d not in ['.git', '__pycache__', 'venv', 'instance', 'migrations']]
        ...
        for filename in filenames:
            if filename.endswith('.py'):
                ...
                try:
                    ...
                    print(f"Erreur lors de l'analyse de {filepath}: {e}", file=sys.stderr)
...
def extract_tokens(code):
    ...
    for token in tokens:
        if token.strip() and not token.isspace():
            ...
            if token in python_keywords or token.isidentifier() or token in '+-*/%=<>!&|^~:,;()[]{}@.':
...
def print_duplicates(duplicates):
    ...
    for i, (hash_val, items) in enumerate(duplicates.items(), 1):
...
def print_imports(imports):
    ...
    print(f"ℹ️  Imports utilisés dans plusieurs fichiers:\n")
...
def main():
    ...
    if not os.path.isdir(args.directory):
        print(f"❌ Erreur: Le répertoire '{args.directory}' n'existe pas.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.check_similar:
        ...
        print(f"   Trouvé {len(similar)} séquences de code similaires dans plusieurs fichiers.")
