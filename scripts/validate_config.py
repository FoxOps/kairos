...
from config import get_bool_from_env, get_int_from_env, get_json_from_env
...
NUMERIC_VARIABLES = {
    "LOG_FILE_SIZE": {"type": "int", "min": 1024, "description": "Taille maximale des fichiers de log en octets"},
    "LOG_BACKUP_COUNT": {"type": "int", "min": 1, "max": 100, "description": "Nombre de fichiers de backup à conserver"},
    "REMEMBER_COOKIE_DURATION": {"type": "int", "min": 0, "description": "Durée du cookie en secondes"},
    "ERROR_503_RETRY_AFTER": {"type": "int", "min": 0, "description": "Délai de réessai pour les erreurs 503 en secondes"},
    "DATABASE_CONNECT_TIMEOUT": {"type": "int", "min": 1, "max": 300, "description": "Délai d'attente pour la connexion à la base de données"},
    "DATABASE_POOL_SIZE": {"type": "int", "min": 1, "max": 50, "description": "Taille du pool de connexions"},
    "DATABASE_MAX_OVERFLOW": {"type": "int", "min": 0, "max": 100, "description": "Nombre maximal de connexions supplémentaires"},
    "SMTP_PORT": {"type": "int", "min": 1, "max": 65535, "description": "Port SMTP"},
    "ICS_TOKEN_EXPIRY_DAYS": {"type": "int", "min": 1, "max": 3650, "description": "Durée de validité du token ICS en jours"},
}
...
class ConfigValidator:
    ...
    def _load_env_file(self, env_file):
        ...
        try:
            ...
            self.info.append(f"Fichier {env_file} chargé manuellement (python-dotenv non disponible)")
    ...
    def validate_critical_variables(self, is_production=False):
        ...
        for var_name, config in CRITICAL_VARIABLES.items():
            ...
            if is_production and config.get("required_in_production"):
                if not value:
                    self.errors.append(f"Variable critique manquante en production: {var_name}")
                    continue
        ...
        for var_name, config in RECOMMENDED_VARIABLES.items():
            ...
            if value:
                ...
                if config.get("type") == "boolean":
                    if value.lower() not in ('true', 'false', '1', '0', 'yes', 'no', 'y', 'n', 'on', 'off'):
                        self.warnings.append(
        ...
        for var_name, config in NUMERIC_VARIABLES.items():
            ...
            if value:
                try:
                    ...
                    self.errors.append(f"{var_name} doit être un entier valide: '{value}'")
        ...
        for var_name in json_vars:
            ...
            if value and value.strip():
                try:
                    ...
                    self.errors.append(f"{var_name} contient un JSON invalide: {str(e)}")
    ...
    def validate_security(self):
        ...
        secret_key = os.environ.get("SECRET_KEY", "ta-cle-secrete-ici")
        
        if flask_env == "production" and secret_key == "ta-cle-secrete-ici":
            self.errors.append(
    ...
    def validate_all(self):
        ...
        try:
            ...
            self.warnings.append(f"Impossible de créer le dossier des logs {log_dir}: {str(e)}")
