...
class TestAddLeaveEdgeCases:
    def test_invalid_user_id(self, test_app, non_admin_client, test_user):
        ...
        assert "vous-m".encode() in resp.data
    ...
    def test_service_exception_handled(self, test_app, logged_in_client, test_user):
        with patch("app.routes.leave_routes.LeaveService.add_leave", side_effect=RuntimeError("boom")):
            resp = logged_in_client.post(
                "/leave/add",
                data={
...
class TestDeleteLeave:
    ...
    def test_delete_leave_exception_handled(self, test_app, logged_in_client, test_leave):
        with patch("app.routes.leave_routes.LeaveService.delete_leave", side_effect=RuntimeError("boom")):
            resp = logged_in_client.get(f"/leave/delete/{test_leave.id}", follow_redirects=True)
        assert resp.status_code == 200
...
class TestApiDeleteLeave:
    ...
    def test_forbidden_for_other_users_leave(self, test_app, non_admin_client, test_leave, second_user):
        ...
        other_leave = Leave(user_id=second_user.id, start_date=date.today(), end_date=date.today())
    ...
    def test_unexpected_exception_returns_500(self, test_app, logged_in_client, test_leave):
        with patch("app.routes.leave_routes.LeaveService.api_delete", side_effect=RuntimeError("boom")):
            resp = logged_in_client.delete(f"/api/leave/{test_leave.id}")
...
class TestApiUpdateLeave:
    def test_not_found(self, test_app, logged_in_client):
        resp = logged_in_client.patch("/api/leave/999999", json={"start": date.today().isoformat()})
        assert resp.status_code == 404

    def test_forbidden_for_other_users_leave(self, test_app, non_admin_client, second_user):
        from app import db
        from app.models import Leave
        ...
        resp = non_admin_client.patch(f"/api/leave/{other_leave.id}", json={"start": date.today().isoformat()})
        assert resp.status_code == 403

    def test_no_json_body(self, test_app, logged_in_client, test_leave):
        resp = logged_in_client.patch(f"/api/leave/{test_leave.id}", data="null", content_type="application/json")
    ...
    def test_success_without_end_uses_original_duration(self, test_app, logged_in_client, test_user):
        ...
        leave = Leave(user_id=test_user.id, start_date=date.today(), end_date=date.today() + timedelta(days=2))
    ...
    def test_invalid_date_format(self, test_app, logged_in_client, test_leave):
        resp = logged_in_client.patch(f"/api/leave/{test_leave.id}", json={"start": "bogus"})
        assert resp.status_code == 400

    def test_unexpected_exception_returns_500(self, test_app, logged_in_client, test_leave):
        with patch("app.routes.leave_routes.LeaveService.api_update", side_effect=RuntimeError("boom")):
            resp = logged_in_client.patch(
