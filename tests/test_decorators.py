...
class TestAdminRequiredDecorator:
    ...
    def test_admin_required_blocks_non_admin(self, logged_in_client):
        ...
        assert b'Acces refuse' in response.data or b'permissions' in response.data.lower() or b'admin' in response.data.lower()
...
class TestUserOwnsResourceDecorator:
    ...
    def test_user_can_delete_own_leave(self, logged_in_client, test_leave, test_user, app):
    ...
    def test_user_cannot_delete_others_leave(self, client, test_leave, second_user, app):
    ...
    def test_admin_can_delete_any_leave(self, logged_in_admin_client, test_leave, app):
        ...
        response = logged_in_admin_client.get(f'/leave/delete/{test_leave.id}', follow_redirects=True)
        assert response.status_code == 200
        assert b'Conge supprime' in response.data or b'succes' in response.data
...
class TestShiftPermissions:
    ...
    def test_admin_can_add_shift(self, logged_in_admin_client, test_user, test_shift_type):
    ...
    def test_non_admin_cannot_add_shift(self, logged_in_client, test_user, test_shift_type):
        ...
        response = logged_in_client.post('/schedule/add', data=data, follow_redirects=True)
        assert response.status_code == 200
...
class TestOnCallPermissions:
    ...
    def test_non_admin_cannot_add_oncall(self, logged_in_client, test_user):
        ...
        data = {
            'user_id': test_user.id,
            'start_date': '2025-12-06'  # Un vendredi
        }
        response = logged_in_client.post('/oncall/add', data=data, follow_redirects=True)
        assert response.status_code == 200
        assert b'Acces refuse' in response.data or b'admin' in response.data.lower() or b'Seuls les administrateurs' in response.data
...
class TestLeavePermissions:
    ...
    def test_user_cannot_add_others_leave(self, client, test_user, second_user, app):
        ...
        with app.app_context():
            ...
            assert b'vous' in response.data.lower() and b'pour vous' in response.data.lower() or b'Seuls' in response.data
    ...
    def test_admin_can_add_leave_for_anyone(self, logged_in_admin_client, test_user):
        ...
        response = logged_in_admin_client.post('/leave/add', data=data, follow_redirects=True)
        assert response.status_code == 200
        assert b'Conge ajoute' in response.data or b'succes' in response.data
